...
void addAccess(struct s_node **branches, int len, char *nodeRepresentation,
...
struct s_node *getBoardFromFile(char *file) {
  ...
  for (int i = 0; lines[i]; i++) {
    if (strcmp(lines[i], "b") == 0) {
      ...
      l += 1;
    } else if (strncmp(lines[i], "m", 1) == 0 && isdigit(lines[i][1])) {
      if (atoi(lines[i] + 1) != l)
  ...
  for (int i = 0; i < n; i++) {
    ...
    while (tmp) {
      printf("%d %s (%d) -> ", tmp->value, (tmp->isFinal) ? "FINAL" : "",
             (tmp->random) ? tmp->random->value : 0);
      tmp = tmp->next;
    }
    printf("END\n");
  }
