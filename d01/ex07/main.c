...
int main(void) {
  ...
  node = genRandomSnakesAndLadders(5); // if the parameter == 0: generate a
                                       // random game board with 1 to 1000 nodes
  nodeCloned = NULL;
  ...
  /*-------------------
    ...
    --------------------*/
  puts("-- Template Board --");
  printSnakeAndLadders(node, stdout);
  nodeCloned = cloneGameBoard(node);
  puts("-- Cloned Board --");
  printSnakeAndLadders(nodeCloned, stderr);
...
void printSnakeAndLadders(struct s_node *node, FILE *fp) {
  while (node) {
    fprintf(fp, "[%d : %p , (%d : %p)]%c", node->value, node,
            (node->random) ? node->random->value : -1, node->random,
            (node->next) ? ' ' : '\n');
    node = node->next;
  }
}
