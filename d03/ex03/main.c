...
int main(void)
{
	...
	root = genMinHeap((unsigned)time(NULL)+(unsigned)clock()); //get a min heap
	...
	fputs("transform the tree to maxHeap...", stdout);
	start = clock();
	saveTheSequoia(&root);
	stop = clock();
	fputs("Done\n", stdout);
	printf("clocks(%lu) time(%f)\n", (stop-start), (double)(stop-start)/CLOCKS_PER_SEC);
	printf("ack(%zu) lswaps(%zu) rswaps(%zu)\n", ack, lswaps, rswaps);
	printBinaryTree(root);
